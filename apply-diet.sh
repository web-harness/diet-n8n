...
echo "=== 0/14 n8n trace (NODE_DEBUG=module) → ${LOGS_TXT} ==="
rm -f "$LOGS_TXT"
...
export N8N_ENCRYPTION_KEY="${N8N_ENCRYPTION_KEY:-diet-build-trace-placeholder-do-not-use-in-prod}"
export N8N_PORT="${N8N_PORT:-5678}"
export N8N_EDITOR_BASE_URL="http://localhost:${N8N_PORT}"
export N8N_HOST=localhost
export N8N_PROTOCOL=http
export N8N_LISTEN_ADDRESS=0.0.0.0
# Diagnostics on for trace only: n8n PostHogClient skips require('posthog-node') when diagnostics is off.
...
export N8N_DISABLED_MODULES=insights
export N8N_NATIVE_PYTHON_RUNNER=false
export N8N_PUBLIC_API_DISABLED=true
export N8N_PUBLIC_API_SWAGGERUI_DISABLED=true
export N8N_REINSTALL_MISSING_PACKAGES=false
...
for _ in $(seq 1 10); do
  node -e "const h=require('http');const port=process.env.N8N_PORT||'5678';['/','/healthz'].forEach((pth)=>{h.get('http://127.0.0.1:'+port+pth,(r)=>r.resume()).on('error',()=>{});});" || true
  sleep 2
done
...
echo "=== 1/14 Build ${DIET_TXT} from node_modules/* paths in ${LOGS_TXT} ==="
...
echo "=== 2/14 Top-level prune (diet.txt + n8n-* / acorn-* keep, same as trace/diet.js) ==="
...
echo "=== 3/14 Linux arm64 native prebuilds (.node + dirs) ==="
...
echo "=== 4/14 Source maps ==="
find "$NODE_MODULES" -name "*.js.map" -delete

echo "=== 5/14 Test data (safe patterns only) ==="
...
echo "=== 9/14 Browser-only bundles ==="
...
echo "=== 11/14 Docs, changelogs, licenses ==="
...
echo "=== 12/14 Config/metadata files ==="
